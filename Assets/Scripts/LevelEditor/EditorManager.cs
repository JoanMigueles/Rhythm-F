...
public class EditorManager : NoteManager
{
    ...
    protected override void Start()
    {
        ...
        if (GameManager.instance.IsSongSelected()) {
            SongMetadata? song = GameManager.instance.GetSelectedSong();
            if (song.HasValue)
        ...
        if (!string.IsNullOrEmpty(songData.metadata.coverFileName)) {
            Sprite coverSprite = SaveData.GetCoverSprite(SaveData.GetCoverFilePath(songData.metadata.coverFileName));
            EditorUI.instance.DisplayCoverImage(coverSprite);
        }
    ...
    public IEnumerator SaveAndLoadCoverFile(string filePath)
    {
        // Save
        SaveData.CreateCoverFile(songData, filePath);
    ...
    {
        if (!EventSystem.current.IsPointerOverGameObject() || draggedNotePreview != null) {
    ...
    public void SetNoteType(string type)
    {
        if (System.Enum.TryParse(type, out NoteType noteType)) {
            ...
            notePreview.GetComponent<SpriteRenderer>().sprite = GetNoteTypePrefab(noteType).GetComponentInChildren<SpriteRenderer>().sprite;
        }
    }
